...
def start(m):
    ...
    logo = settings.get("logo_url", LOGO_URL)
    bot_name = settings.get("bot_name", "🛍️ ANTO SHOP")
...
def manual_pay(c):
    ...
    number = settings.get("bkash", BKASH_NUMBER) if c.data == "pay_bkash" else settings.get("nagad", NAGAD_NUMBER)
...
📌 Instructions:
1️⃣ Send {settings.get('currency', '৳')}{pend['price']} to {number}
2️⃣ Send TRX ID
...
⏰ Time: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"""
    ...
    bot.send_message(m.chat.id, "✅ Order sent to admin! Please wait for approval.", parse_mode="Markdown")
...
📱 BKash: `{settings.get('bkash', BKASH_NUMBER)}`
📱 Nagad: `{settings.get('nagad', NAGAD_NUMBER)}`
...
def del_product(m):
    ...
    try:
        ...
        bot.reply_to(m, "❌ Use: `/delproduct PID`", parse_mode="Markdown")
...
def add_balance_admin(m):
    ...
    try:
        ...
        bot.reply_to(m, "❌ Use: `/addbalance UID AMOUNT`", parse_mode="Markdown")
