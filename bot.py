...
CHANNEL_LINK = "https://t.me/+G9DiZ6NznoQ2Yzc1"  # Your channel link
...
def add_balance(user_id, amount):
    if str(user_id) not in wallet_db:
        wallet_db[str(user_id)] = {"balance": 0, "transactions": []}
    wallet_db[str(user_id)]["balance"] += amount
    wallet_db[str(user_id)]["transactions"].append({
        "type": "add",
        "amount": amount,
...
def deduct_balance(user_id, amount):
    if str(user_id) not in wallet_db:
        return False
    if wallet_db[str(user_id)]["balance"] >= amount:
        wallet_db[str(user_id)]["balance"] -= amount
        wallet_db[str(user_id)]["transactions"].append({
            "type": "deduct",
            "amount": amount,
...
✅ Premium Gaming & Streaming Services
✅ Instant Delivery
✅ 24/7 Support
✅ Wallet System

💰 Your Balance: ৳{wallet_db.get(user_id, {}).get('balance', 0)}
...
def purchase(call):
    ...
    user_balance = wallet_db.get(str(call.message.chat.id), {}).get('balance', 0)
...
💵 Your Wallet Balance: ৳{user_balance}
...
💝 Thank you for shopping with ANTO SHOP!"""
        ...
        del pending_purchase[str(call.message.chat.id)]
    else:
        bot.answer_callback_query(call.id, "❌ Insufficient balance! Please add balance first.")
...
⚠️ Use the reference number when sending!"""
...
⚠️ Send amount and TRX ID in one message!"""
    ...
    bot.edit_message_caption(text, call.message.chat.id, call.message.message_id,
                            reply_markup=markup, parse_mode="Markdown")
    bot.send_message(call.message.chat.id, "📝 **Send your amount and TRX ID:**\n\nExample: `500 8Y7X9K2M4N`")
...
🔢 TRX ID: `{trx}`"""
        ...
        bot.send_message(message.chat.id, "✅ Request sent to admin! Please wait.", parse_mode="Markdown")
    except:
...
def approve_balance(call):
    ...
    bot.send_message(uid, f"✅ ৳{amount} added to your wallet!", parse_mode="Markdown")
...
def profile(call):
    ...
    balance = wallet_db.get(str(call.message.chat.id), {}).get('balance', 0)
...
def lucky_spin(call):
    ...
    # Prize amounts (real money to wallet)
    prizes = [5, 10, 20, 50, 100, 0, 0, 5, 10, 0]
...
`/addduration PID|DUR_ID|NAME|PRICE` - Add duration
...
`/deductbalance UID|AMOUNT` - Deduct balance
`/wallet UID` - View user wallet

━━━━━━━━━━━━━━━━━━━━
👥 **USER MANAGEMENT:**
`/users` - List all users
`/user UID` - View user details
...
def list_products(message):
    ...
    for pid, p in products_db.items():
        text += f"🆔 **{pid}** - {p['name']}\n"
        text += f"   📅 Durations:\n"
        for did, d in p['durations'].items():
...
def add_product(message):
    ...
    try:
        _, name, price, desc = message.text.split("|")
        new_id = str(len(products_db) + 1)
        products_db[new_id] = {
            "name": name,
            "price": int(price),
            "durations": {
                "default": {"name": "Standard", "price": int(price)}
            },
            "desc": desc,
            "keys": [],
            "stock": 0
        ...
        save_db('products.json', products_db)
        bot.reply_to(message, f"✅ Product added! ID: {new_id}")
    except:
        bot.reply_to(message, "❌ /addproduct Name|Price|Description")

@bot.message_handler(commands=['addduration'])
def add_duration(message):
    if message.chat.id != ADMIN_ID:
        return
    
    try:
        _, pid, dur_id, name, price = message.text.split("|")
        if pid in products_db:
            products_db[pid]['durations'][dur_id] = {"name": name, "price": int(price)}
            save_db('products.json', products_db)
            bot.reply_to(message, f"✅ Duration added to product {pid}!")
        else:
            bot.reply_to(message, "❌ Product not found!")
    except:
        bot.reply_to(message, "❌ /addduration PID|dur_id|Name|Price")
...
def list_users(message):
    ...
    for uid, user in list(users_db.items())[:30]:
        balance = wallet_db.get(uid, {}).get('balance', 0)
        text += f"🆔 `{uid}` | {user.get('name', 'Unknown')}\n   💰 ৳{balance} | 📦 {len(user.get('orders', []))}\n\n"
    bot.send_message(message.chat.id, text, parse_mode="Markdown")
...
def add_balance_admin(message):
    ...
    try:
        ...
        bot.send_message(int(uid), f"💰 ৳{amount} added to your wallet by admin!")
